...
def _read_session(db, session_id: str, head: int = 20, tail: int = 10, scope=None,
                  link_profile: str = None) -> str:
...
def _list_recent_sessions(db, limit: int, current_session_id: str = None, scope=None,
                          link_profile: str = None) -> str:
...
def _session_search_impl(
    ...
    *,
    ...
    if isinstance(session_id, str) and session_id.strip():
        ...
        result = _read_session(db, sid, scope=scope, link_profile=profile)
        ...
        if located is not None:
            try:
                found = json.loads(_read_session(located, sid, scope=scope, link_profile=owner))
            finally:
                located.close()
    ...
    if not query or not isinstance(query, str) or not query.strip():
        return _list_recent_sessions(db, limit, current_session_id, scope=scope, link_profile=profile)
