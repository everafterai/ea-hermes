...
def run_job(
    ...
    *,
    ...
    try:
        ...
        # Model resolution precedence: per-job override > skill frontmatter
        # override > cron.model (the cron-fleet default) > HERMES_MODEL env >
        # config.yaml ``model:`` (string or ``{default: ...}``). The per-job
        # value is intentionally re-read from storage every tick so a
        # ``hermes cron edit --model`` after a failed run takes effect on the
        ...
        try:
            ...
            if os.path.exists(_cfg_path):
                ...
                # Gate on _eff_model, not job.get("model"): a skill listed on
                # the job may supply the model via SKILL.md frontmatter, and
                # that override outranks the cron-fleet/global defaults just
                # as an explicit per-job model does.
                if not _eff_model:
        ...
        try:
            ...
            runtime_kwargs = {
                # Per-job user pin wins, then a skill's frontmatter override
                # (_eff_provider covers both), then the cron-fleet default
                # provider (cron.model_provider), then persisted global config.
                "requested": _eff_provider or _cron_default_provider or None,
