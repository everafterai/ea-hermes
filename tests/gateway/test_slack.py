...
# ---------------------------------------------------------------------------
# Fork features carried onto the bundled Slack plugin adapter
# (quiet channels, slack_react targeting, relevance-gate bypass).
# ---------------------------------------------------------------------------


class TestForkSlackAdapterFeatures:
    @pytest.mark.asyncio
    async def test_reactions_skipped_in_quiet_channel(self, adapter):
        ...
        marker = adapter._workspace_message_marker("", "ts_quiet")
    ...
    def test_anchor_message_author_labels_channel_messages(self):
        from plugins.platforms.slack.adapter import _anchor_message_author

        assert _anchor_message_author("hi", "Alice", False) == "[Message from Alice]\nhi"
        # DMs are unambiguous; blank author/body are never labelled.
        assert _anchor_message_author("hi", "Alice", True) == "hi"
        assert _anchor_message_author("hi", "", False) == "hi"
        assert _anchor_message_author("   ", "Alice", False) == "   "
