...
class GatewayAuthorizationMixin:
    ...
    def _is_user_authorized(
        ...
        *,
        ...
        #
        # Placed after the plugin-registry env-map lookup above (so the
        # "ignored" warning names the right variable) and BEFORE the allow-all
        # check below, because RBAC must override allow-all, not lose to it.
        try:
            ...
            if _rbac_policy.enabled:
                _env_var = platform_env_map.get(source.platform, "")
                if _env_var and _auth_env(_env_var).strip():
