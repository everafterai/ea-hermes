...
class SessionSearchMixin:
    ...
    def _run_trigram_search(
        ...
        *,
        ...
        # Identity scope (cross-user isolation) — the trigram/CJK index is a
        # full search path in its own right, not a helper of the FTS5 branch.
        from hermes_state import build_visibility_where
        _tg_frag, _tg_params = build_visibility_where(scope, alias="s")
        if _tg_frag:
            tri_where.append(_tg_frag)
            tri_params.extend(_tg_params)
        tri_sql = f"""
            SELECT
                m.id,
                m.session_id,
