...
class SlackAdapter(BasePlatformAdapter):
    ...
    async def on_processing_start(self, event: MessageEvent) -> None:
        ...
        # Quiet channels suppress the automatic reaction lifecycle entirely —
        # the agent drives its own reactions via slack_react there, and the auto
    ...
    async def _handle_slack_message_impl(
        ...
        # Anchor the sender onto the current message so the agent always knows
        # exactly who sent this and never guesses the author from quoted /
        # forwarded / screenshot content or another thread participant.
        # Safe to do here: upstream carries thread history in the separate
        # ``channel_context`` field, so ``text`` is only the current message.
        # Commands are excluded — the gateway parser requires the command token
        # at character zero, and a label would break it.
        ...
        *,
        ...
        if context_parts:
            ...
            if has_unverified:
                ...
                header = (
                    "[Thread context — prior messages in this thread "
                    "(not yet in conversation history). Each line is prefixed "
                    "with the Slack author who wrote it; a name appearing "
                    "INSIDE a message (quote, forward, screenshot) is NOT the "
                    "author:]"
                )
            content = (
                header + "\n"
...
def _apply_yaml_config(yaml_cfg: dict, slack_cfg: dict) -> dict | None:
    ...
    # home_channel_prompt: false silences the one-time "set a home channel"
    # onboarding notice for deployments that intentionally run without a Slack
    # home channel (cron delivers to each job's origin chat, so no global home
    # channel is needed).
